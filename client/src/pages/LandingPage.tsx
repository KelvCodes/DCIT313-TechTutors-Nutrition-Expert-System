...
const LandingPage = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen w-full relative overflow-x-hidden bg-white">
      {/* Decorative Blobs */}
      <div className="absolute top-[-100px] right-[-100px] w-[500px] h-[500px] bg-sea-blue-light blur-[80px] rounded-full opacity-15 z-0" />
      <div className="absolute bottom-[100px] left-[-100px] w-[500px] h-[500px] bg-sea-blue blur-[80px] rounded-full opacity-15 z-0" />

      <Header />

      <main className="flex-1 flex flex-col relative z-10 w-full max-w-7xl mx-auto px-0 sm:px-8">
        <section id="home" className="pt-28 sm:pt-40 md:pt-48 pb-12 sm:pb-24 flex flex-col md:flex-row items-center gap-10 md:gap-16 px-4 sm:px-0">
          ...
          <div className="flex-1 text-center md:text-left animate-fade-in-up">
            ...
            <div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 justify-center md:justify-start">
              <button
                ...
                className="bg-gradient-sea text-white px-7 py-3.5 rounded-xl font-bold text-base sm:text-lg shadow-lg hover:shadow-sea-blue/30 transition-all hover:-translate-y-1 active:scale-95">
                Start Your Journey
              </button>
              <button
                onClick={() => document.getElementById('about')?.scrollIntoView({ behavior: 'smooth' })}
                className="bg-white/50 backdrop-blur-sm border border-slate-200 text-slate-900 px-7 py-3.5 rounded-xl font-bold text-base sm:text-lg hover:bg-white transition-all hover:border-sea-blue/30 group">
                Watch Demo
          ...
          <div className="flex-1 relative animate-float w-full max-w-sm sm:max-w-none mt-4 md:mt-0">
            <div className="absolute inset-0 bg-gradient-sea rounded-[40px] blur-3xl opacity-20 -z-10 transform scale-90" />
