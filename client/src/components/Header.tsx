  ...
  return (
    <header className="fixed top-0 sm:top-8 left-0 sm:left-8 right-0 sm:right-8 z-[1000] rounded-none sm:rounded-[18px] bg-[rgba(255,255,255,0.7)] sm:bg-[rgba(255,255,255,0.45)] backdrop-blur-xl sm:backdrop-blur-[18px] border-b sm:border border-[rgba(255,255,255,0.4)] shadow-sm sm:shadow-[0_4px_30px_rgba(0,0,0,0.05)] transition-all">
      <div className="flex items-center justify-between px-4 sm:px-8 py-3.5 sm:py-4">
        {/* Logo */}
        <button onClick={() => navigate('/')} className="text-xl sm:text-2xl font-bold bg-gradient-sea bg-clip-text text-transparent hover:opacity-80 transition-opacity">
          Nutrix-AI
        </button>
        {/* Desktop nav */}
        ...
        <button 
          ...
          className="hidden md:flex bg-gradient-sea text-white px-5 py-2.5 rounded-full font-semibold text-sm transition-all hover:-translate-y-0.5 hover:shadow-[0_4px_12px_rgba(0,119,190,0.3)]">
          Get Started
        </button>
      ...
      {menuOpen && (
        <div className="md:hidden px-5 pb-5 flex flex-col gap-4 border-t border-white/30 mt-1 pt-4">
          ...
          <button 
            onClick={() => { setMenuOpen(false); navigate('/consult'); }}
            className="bg-gradient-sea text-white px-5 py-3 rounded-xl font-semibold text-sm w-full mt-1">
            Get Started
          </button>
        </div>
      )}
